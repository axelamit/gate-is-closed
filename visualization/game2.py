...
screen = pygame.display.set_mode([blocksize * 10 * drawing_scale, blocksize * 10 * drawing_scale])
        ...
        for j in range(n_connections):
            ...
            if (main_x == next_x):
                if (wire_state == 1):
                    ...
                    cable = pygame.transform.scale(cable, (int(blocksize * drawing_scale ), int((abs(main_y - next_y) + 1) * blocksize * drawing_scale)))
                    cable.convert()
                    screen.blit(cable, (main_x * blocksize * drawing_scale, min(main_y, next_y) * blocksize * drawing_scale))
                else:
                ...
                if (wire_state == 1):
                    ...
                    cable = pygame.transform.scale(cable, (int((abs(main_x - next_x) + 1) * blocksize * drawing_scale), int(blocksize * drawing_scale)))
                    cable.convert()
                    screen.blit(cable, (min(main_x, next_x) * blocksize * drawing_scale, main_y * blocksize * drawing_scale))
                else:
